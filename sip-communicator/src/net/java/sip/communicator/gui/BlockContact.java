...
public class  BlockContact extends JDialog
{
	 ...
	 */
	public BlockContact(Frame parent, boolean modal)
	{
		super(parent, modal);
		initComponents();
		pack();
	...
	private void initComponents()
	{
		...
		// have a description
		getAccessibleContext().setAccessibleDescription("Block");

		String authPromptLabelValue = Utils.getProperty("net.java.sip.communicator.gui.AUTHENTICATION_PROMPT");


		authPromptLabelValue  = "Please enter the user's name you want to block";
		...
		submitButton.addActionListener(new ActionListener()
		{
			...
			{	
				String blockee = userNameTextField.getText();
				String blocker = Global.currentUser;
				JFrame frame = new JFrame();
				if (blockee.equals("") || (blockee==null))  {
					...
					if (blockman.checkIfUserExists(blockee)){
						if (blockman.addBlockToDB(blocker, blockee)){
							JOptionPane.showMessageDialog(frame, "Your calls will be blocked from "+blockee,"Blocking Status", JOptionPane.INFORMATION_MESSAGE);
							dialogDone(event);
						}
						else{

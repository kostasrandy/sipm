...
//
	...
	public void registerToProxies() {
		try {
			...
			if (proxyToRegisterWithList != null) {
				...
				if (ProxyDebug.debug) {
					ProxyDebug
							.println("Registrar, registerToProxies(), we have to register to "
									+ proxyToRegisterWithList.size()
									+ " proxies");
				}
				for (int i = 0; i < proxyToRegisterWithList.size(); i++) {
					Domain domain = (Domain) proxyToRegisterWithList
							.elementAt(i);
					if (domain.hostName != null) {
						RegistrationDomainThread rr = new RegistrationDomainThread(
								proxy, domain);
						// ifdef SIMULATION
						/*
						 * new SimThread(rr).start(); //else
						 */
						new Thread(rr).start();
						// endif
						//
			...
			if (ProxyDebug.debug) {
				ProxyDebug
						.println("ERROR, Registrar, registerToProxies(), exception  raised:");
			}
			e.printStackTrace();
		}
	}
	...
	public void parseXMLregistrations(String file) {
		try {
			...
			XMLRegistrationsParser xmlRegistrationsParser = new XMLRegistrationsParser(
					xmlRegistrationsFile, proxy);
			Registrations registrations = xmlRegistrationsParser
					.getRegistrations();
			...
			if (registrationList != null) {
				if (ProxyDebug.debug) {
					ProxyDebug
							.println("Registrar, parseXMLregistrations(), Uploading of "
									+ registrationList.size()
									+ " registrations");
				}
				for (int i = 0; i < registrationList.size(); i++) {
					Registration registration = (Registration) registrationList
							.elementAt(i);
			...
			if (ProxyDebug.debug) {
				ProxyDebug
						.println("ERROR, Registrar, Registrar(), exception  raised during"
	...
	public void clean() {
		...
		for (int i = 0; i < threads.size(); i++) {
			RegistrationDomainThread rr = (RegistrationDomainThread) threads
					.elementAt(i);
			rr.STOP = true;
		}
	}
	...
	public void initRMIBindings() {
		...
		try {
			...
			if (configuration.accessLogViaRMI) {
				...
				if (ProxyDebug.debug)
					ProxyDebug
							.println("We don't export the registrations because RMI is disabled.");
			}
		} catch (Exception ex) {
			if (ProxyDebug.debug) {
				ProxyDebug
						.println("Problem trying to export the Registration Table: "
								+ name);
			}
			ex.printStackTrace();
		}
	...
	public synchronized void processRegister(Request request,
		...
		try {
			...
			System.out.println("I am in processRegister and the key is " + key);
	...
	/**
	 * The key is built following this rule: The registrar extracts the
	...
	public String getKey(Request request) {
		...
		try {
			...
			if (ProxyDebug.debug) {
				ProxyDebug
						.println("Registrar, hasDomainRegistered(), internal error, "
								+ "exception raised:");
				ProxyDebug.logException(ex);
			}
			return null;
	...
	public Vector getDomainContactsURI(Request request) {
		try {
			...
			if (ProxyDebug.debug) {
				ProxyDebug
						.println("Registrar, getDomainContacts(), internal error, "
								+ "exception raised:");
				ProxyDebug.logException(ex);
			}
			return null;
	...
	public boolean hasRegistration(Request request) {
		try {
			...
			if (ProxyDebug.debug) {
				ProxyDebug
						.println("Registrar, hasRegistration(), internal error, "
								+ "exception raised:");
				ProxyDebug.logException(ex);
			}
			return false;
	 ...
	 * the ToHeader URI from this request.
	 */
	public Vector getContactsURI(Request request) {
		try {
			...
			if (ProxyDebug.debug) {
				ProxyDebug
						.println("Registrar, getContactsURI(), internal error, exception raised:");
				ProxyDebug.logException(ex);
			}
			return null;
		}
	...
	public boolean hasContactHeaders(Request request) {
		ListIterator list = (ListIterator) request
	...
	private boolean hasExpiresZero(Request request) {
		try {
			...
			if (expiresHeader == null) {
				ProxyDebug
						.println("Registrar, hasExpiresZero(), the REGISTER does not have an Expires Header");
				return false;
			} else {
				ProxyDebug
						.println("Registrar, hasExpiresZero(), the REGISTER has an Expires Header with"
			...
			if (ProxyDebug.debug) {
				ProxyDebug
						.println("Registrar, hasExpiresZero(), internal error, exception raised:");
	...
	public static Vector getContactHeaders(Request request) {
		...
		try {
			...
			if (ProxyDebug.debug) {
				ProxyDebug
						.println("Registrar, getContactHeaders(), internal error, exception raised:");
				ProxyDebug.logException(e);
			}
			return contacts;
		}
